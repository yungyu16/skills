...
```bash
# 基本搜索
node scripts/search.js "Python 异步编程"

# 指定结果数
node scripts/search.js "AI 新闻" --num 10

# 时间过滤（本周内）
node scripts/search.js "科技动态" --tbs qdr:w
...
搜索结果只有标题和摘要，**需要配合 `read.js` 阅读具体内容**：

```bash
# 1. 搜索找到相关 URL
node scripts/search.js "Python 最佳实践 2024" -o search.json
